...
with tab_paper:
    ...
    if _pred and isinstance(_pred, dict):
        ...
        # Intraday reality-check helpers. Sit directly under each
        # next-day probability so the user can compare the model's
        # prediction with what the market is actually doing right now.
        # Cached 60s to avoid hammering yfinance.
        @st.cache_data(ttl=60, show_spinner=False)
        def _intraday_snapshot(ticker: str) -> dict:
            """Open + current price for a ticker, with closed-market fallback."""
            try:
        ...
        def _render_intraday_card(snap: dict):
            ...
            closed_note = (
                " <span style='color:#9ca3af;font-size:0.78em;'>"
            ...
            st.markdown(
                ...
                f"<div style='margin-top:4px;font-size:0.95em;'>"
        ...
        with _m_l:
            if wti and "prob_up" in wti:
                ...
                _render_intraday_card(_intraday_snapshot("CL=F"))
        with _m_r:
            if ng and "prob_up" in ng:
                ...
                _render_intraday_card(_intraday_snapshot("NG=F"))
